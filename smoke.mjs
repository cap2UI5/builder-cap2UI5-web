// Browser smoke test — catches the failure mode this site has actually
// shipped with more than once: a silent blank page (broken UI5 bootstrap,
// CDN 404, bundle not injected). Opens the site in headless Chromium and
// asserts the real startup path:
//
//   1. the in-browser backend bundle is loaded (window.fetch is intercepted)
//   2. UI5 boots from the CDN
//   3. the startup roundtrip is answered in-process and UI5 renders content
//
// Two modes:
//   npm run smoke                    build output: serves dist/ locally
//   SMOKE_URL=https://… npm run smoke   live site: tests the given base URL
//                                        directly (no local server, no dist/
//                                        needed) — see also live-smoke.mjs
//
// SMOKE_URL may include a subpath (GitHub Pages project sites live under
// https://<org>.github.io/<repo>/) — all navigation is relative to it.
//   CHROMIUM_PATH=/path/to/chromium overrides the browser binary (e.g. a
//   preinstalled one when Playwright's own download is unavailable).

import { createServer } from "node:http";
import fs from "node:fs";
import path from "node:path";
import { pathToFileURL } from "node:url";
import { chromium } from "playwright";
import { DIST_DIR } from "./paths.mjs";

// UI5 truly booted AND rendered: the loader global exists and an sap.m
// control is in the DOM. Deliberately NOT '[class*="sapUi"]' — the static
// index.html body already carries sapUiBody/sapUiSizeCompact, which would
// satisfy that selector on a completely blank page (dead CDN, broken
// bootstrap) and turn the check vacuous. A rendered sap.m control can only
// come from the framework actually running the startup app.
async function waitForUi5Render(page) {
  try {
    await page.waitForFunction(
      () => !!globalThis.sap?.ui && !!document.querySelector('[class*="sapM"]'),
      { timeout: 120000 },
    );
  } catch {
    const state = await page
      .evaluate(() => ({
        ui5: !!globalThis.sap?.ui,
        version: globalThis.sap?.ui?.version ?? null,
        rendered: !!document.querySelector('[class*="sapM"]'),
      }))
      .catch(() => null);
    throw new Error(
      `UI5 did not render within 120 s — ${
        state
          ? `UI5 loaded: ${state.ui5} (version ${state.version}), sap.m control rendered: ${state.rendered}`
          : "page state unavailable"
      } (blank page — broken bootstrap or CDN failure?)`,
    );
  }
}

// Runs the full smoke assertion suite against `base` (site base URL, no
// trailing slash — may include a subpath). Throws on the first failure.
export async function runSmoke(base) {
  const browser = await chromium.launch({
    args: ["--no-sandbox"],
    ...(process.env.CHROMIUM_PATH ? { executablePath: process.env.CHROMIUM_PATH } : {}),
  });
  const page = await browser.newPage();
  const consoleErrors = [];
  page.on("console", (m) => m.type() === "error" && consoleErrors.push(m.text()));
  page.on("pageerror", (e) => consoleErrors.push(String(e)));

  let failure = null;
  try {
    const resp = await page.goto(`${base}/index.html`, { waitUntil: "domcontentloaded", timeout: 30000 });
    if (!resp.ok()) throw new Error(`index.html answered ${resp.status()}`);

    // (1) bundle active: entry.mjs replaced window.fetch before UI5 boots
    const intercepted = await page.evaluate(() => !/\[native code\]/.test(String(window.fetch)));
    if (!intercepted) throw new Error("fetch interceptor not installed — bundle missing or broken");

    // (2)+(3) UI5 booted and rendered the startup app (in-process roundtrip
    // answered) — a blank page fails this check, which is the point.
    await waitForUi5Render(page);

    // (4) wire contract, in-page (the interceptor answers fetch in-process —
    // it matches on the trailing /rest/root/z2ui5 path, so this works both at
    // the domain root and under a Pages subpath and never touches the
    // network — patterned on upstream's node/tests/e2e/roundtrip.spec.js):
    //     app_start → APP + draft id, then draft chaining via S_FRONT.ID
    const wire = await page.evaluate(async () => {
      const post = (sFront) =>
        fetch("/rest/root/z2ui5", {
          method: "POST",
          body: JSON.stringify({
            value: { S_FRONT: { ORIGIN: location.origin, PATHNAME: location.pathname, SEARCH: "", HASH: "", ...sFront } },
          }),
        }).then((r) => r.json());
      const first = await post({ SEARCH: "?app_start=z2ui5_cl_ui5_app_hi_world" });
      const second = await post({ ID: first.S_FRONT.ID });
      const head = await fetch("/rest/root/z2ui5", { method: "HEAD" });
      return {
        firstApp: first.S_FRONT.APP,
        firstId: first.S_FRONT.ID,
        secondApp: second.S_FRONT.APP,
        secondId: second.S_FRONT.ID,
        csrf: head.headers.get("x-csrf-token"),
      };
    });
    const uuid = /^[0-9a-f-]{36}$/;
    if (wire.firstApp !== "z2ui5_cl_ui5_app_hi_world") throw new Error(`app_start answered APP=${wire.firstApp}`);
    if (!uuid.test(wire.firstId)) throw new Error(`app_start draft id malformed: ${wire.firstId}`);
    if (wire.secondApp !== "z2ui5_cl_ui5_app_hi_world") throw new Error(`draft chaining lost the app: ${wire.secondApp}`);
    if (!uuid.test(wire.secondId) || wire.secondId === wire.firstId)
      throw new Error(`draft chaining did not answer a fresh draft id (${wire.secondId})`);
    if (wire.csrf !== "disabled") throw new Error(`HEAD did not answer x-csrf-token: disabled (got ${wire.csrf})`);

    // (5) app start through the real shell: URL-driven app_start renders and
    // leaves no dangling '#' (upstream HashChanger regression)
    await page.goto(`${base}/index.html?app_start=z2ui5_cl_ui5_app_hi_world`, { waitUntil: "domcontentloaded" });
    await waitForUi5Render(page);
    await page.waitForTimeout(250);
    if (page.url().includes("#")) throw new Error(`dangling '#' after app start: ${page.url()}`);

    console.log(`smoke OK against ${base} — bundle active, UI5 booted, startup app rendered, roundtrip/draft-chain/CSRF/app_start verified`);
  } catch (e) {
    failure = e;
  } finally {
    await browser.close();
  }
  if (failure) {
    if (consoleErrors.length) console.error("browser console errors:\n  " + consoleErrors.join("\n  "));
    throw failure;
  }
}

// Serves dist/ on an ephemeral local port; returns { origin, close }.
export function serveDist() {
  if (!fs.existsSync(path.join(DIST_DIR, "index.html"))) {
    throw new Error("dist/index.html missing — run `npm run build` first");
  }
  const MIME = { ".html": "text/html", ".js": "text/javascript", ".json": "application/json", ".css": "text/css", ".properties": "text/plain", ".png": "image/png", ".svg": "image/svg+xml" };
  const server = createServer((req, res) => {
    const urlPath = decodeURIComponent(req.url.split(/[?#]/)[0]);
    let file = path.normalize(path.join(DIST_DIR, urlPath));
    if (!file.startsWith(DIST_DIR)) return res.writeHead(403).end();
    if (fs.existsSync(file) && fs.statSync(file).isDirectory()) file = path.join(file, "index.html");
    if (!fs.existsSync(file)) return res.writeHead(404).end();
    res.writeHead(200, { "Content-Type": MIME[path.extname(file)] || "application/octet-stream" });
    fs.createReadStream(file).pipe(res);
  });
  return {
    listen: () =>
      new Promise((resolve) =>
        server.listen(0, "127.0.0.1", () => resolve(`http://127.0.0.1:${server.address().port}`)),
      ),
    close: () => server.close(),
  };
}

// CLI: SMOKE_URL set → test that base URL directly; otherwise serve dist/.
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const live = process.env.SMOKE_URL;
  if (live) {
    await runSmoke(live.replace(/\/+$/, ""));
  } else {
    const server = serveDist();
    const origin = await server.listen();
    try {
      await runSmoke(origin);
    } finally {
      server.close();
    }
  }
}
